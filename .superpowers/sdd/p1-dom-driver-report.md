...
- The controller provides readonly `canGoNext`, `canGoPrevious`, and `canCancel` callbacks plus internal capability-change observation. Scoped next/back/cancel controls synchronize native `disabled` and `aria-disabled`; denied shortcuts remain unconsumed.
